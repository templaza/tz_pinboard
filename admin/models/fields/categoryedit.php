...
class JFormFieldCategoryEdit extends JFormFieldList
{
	...
	protected function getOptions()
	{
		...
		if ($this->element['parent'] ||  ($jinput->get('option') == 'com_tz_pinboard' && $jinput -> get('view') == 'category'))
		{
		...
		// For items the old category is the category they are in when opened or 0 if new.
		{
			...
			$extension = $this->element['extension'] ? (string) $this->element['extension'] : (string) $jinput->get('option','com_tz_pinboard');
		}
		...
		$query->from('#__tz_pinboard_category AS a');
		$query->join('LEFT', $db->quoteName('#__tz_pinboard_category').' AS b ON a.lft > b.lft AND a.rgt < b.rgt');

		// Filter by the extension type
		if ($this->element['parent'] == true || $jinput->get('option') == 'com_tz_pinboard')
		{
		...
		// If parent isn't explicitly stated but we are in com_tz_pinboard assume we want parents
		if ($oldCat != 0 && ($this->element['parent'] == true || ($jinput->get('option') == 'com_tz_pinboard'
                                                                  AND $jinput -> get('view') == 'categories')))
		{
		// Prevent parenting to children of this item.
		// To rearrange parents and children move the children up, not the parents down.
			$query->join('LEFT', $db->quoteName('#__tz_pinboard_category').' AS p ON p.id = '.(int) $oldCat);
			$query->where('NOT(a.lft >= p.lft AND a.rgt <= p.rgt)');

			$rowQuery	= $db->getQuery(true);
			$rowQuery->select('a.id AS value, a.title AS text, a.level, a.parent_id');
			$rowQuery->from('#__tz_pinboard_category AS a');
