 ...
 */
class TZ_PinboardModelCategory extends JModelAdmin
{
	/**
...
?>
        <select id="jform_params_tz_fieldsid" multiple="multiple"
            ...
            <option value=""<?php if(in_array(-1,$fieldsId)) echo ' selected="selected"';?>><?php echo JText::_('COM_TZ_PINBOARD_ALL_FIELDS');?></option>
            <?php if($list):?>
...
<?php
    ...
    public function getFieldsGroup(){
        ...
        $query          = 'SELECT * FROM #__tz_pinboard_fields_group';
        $dbo -> setQuery($query);

        if(!$rows2 = $dbo -> loadObjectList()){
            $fieldsgroup  = '<select name="groupid[]" size="1" style="min-width: 130px;" id="groupid">';
            $fieldsgroup  .= '<option value="">'.JText::_('COM_TZ_PINBOARD_OPTION_SELECT_FIELDS_GROUP').'</option>';
            $fieldsgroup  .= '</select>';
            return $fieldsgroup;
        }
	...
	*/
	...
	protected function populateState()
	{
		...
		$extension = JRequest::getCmd('extension', 'com_tz_pinboard');
...
//		$params = JComponentHelper::getParams('com_categories');
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_tz_pinboard.category' . $extension, 'category', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_tz_pinboard.edit.' . $this->getName() . '.data', array());
	...
	public function save($data)
	{
		...
		$table = JTable::getInstance('Tz_Pinboard_category','Table');
	...
	*/
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		$query->from($db->quoteName('#__tz_pinboard_category'));
		$db->setQuery($query);
		$count = $db->loadResult();
...
//                    $currentId  = implode(',',$oldParentId);
                ...
                $val    = null;
                $query2 = 'SELECT * FROM #__tz_pinboard_categories'
                ...
                if(isset($val) && $val && count($val)>0){
                    $val    = implode(',',$val);
                    $query2 = 'INSERT INTO #__tz_pinboard_categories(`groupid`,`catid`,`images`)'
                              .' VALUES '.$val;
                    $db -> setQuery($query2);
                    if(!$db -> query()){
                        $this -> setError($db -> getErrorMsg());
	...
	/**
	 * Custom clean the cache of com_tz_pinboard and content modules
	 *
	 * @since	1.6
	 */
