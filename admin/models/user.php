 ...
 *
 */
class TZ_PinboardModelUser extends JModelAdmin
{
...
//        var_dump($result);
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_tz_pinboard.user', 'user', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_tz_pinboard.edit.user.data', array());
    ...
    function saveUser($data){
        ...
        $value  = '('.$data['usersid'].',"'
                  ...
                  .$data['facebook'].'","'
                  .$data['google_one'].'")';
        $query  = 'INSERT INTO #__tz_pinboard_users(`usersid`,`images`,`url`,`gender`,`description`,`twitter`,`facebook`,`google_one`)'
            .' VALUES '.$value;
...
//        var_dump($deleteImage); die();
        ...
        if(!empty($avatar['name']['TZclient_images'])){
            $image  = $avatar;
        }
        else{
