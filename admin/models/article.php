...
//
//        $query  = 'SELECT f.id,f.title FROM #__tz_pinboard_fields AS f'
//                  .' LEFT JOIN #__tz_pinboard_xref AS x ON x.fieldsid=f.id'
...
//        $query  = 'SELECT contentid,groupid FROM #__tz_pinboard_xref_content'
//                  .$where;
//        $db     = &JFactory::getDbo();
//        $db -> setQuery($query);
//        if(!$db -> query()){
...
//        }
//        $query  = 'SELECT * FROM #__tz_pinboard_fields'.$where;
//        $db     = &JFactory::getDbo();
//        $db -> setQuery($query);
//        if(!$db -> query()){
//            var_dump($db -> getErrorMsg());
...
//
//    $db     = &JFactory::getDbo();
//    $query  = 'SELECT attachfiles,attachtitle FROM #__tz_pinboard_xref_content'
...
//    $title  = implode('///',$title);
//
//    $query  = 'UPDATE #__tz_pinboard_xref_content'
...
//
//        if($this -> contentid){
//            $query  = 'SELECT * FROM #__tz_pinboard'
...
//        $data   = array();
//        if($this -> contentid){
//            $query  = 'SELECT attachfiles,attachtitle,attachold FROM #__tz_pinboard_xref_content'
//                .' WHERE contentid = '.$this -> contentid;
//            $db     = &JFactory::getDbo();
//            $db -> setQuery($query);
//            if(!$db -> query()){
...
//        // if inherit category
//        if($groupId == 0){
//            $query  = 'SELECT f.* FROM #__tz_pinboard_fields AS f'
//                      .' LEFT JOIN #__tz_pinboard_xref AS x ON x.fieldsid=f.id'
//                      .' LEFT JOIN #__tz_pinboard_categories AS c ON c.groupid = x.groupid'
...
//        else{
//            $query  = 'SELECT f.* FROM #__tz_pinboard_fields AS f'
//                      .' LEFT JOIN #__tz_pinboard_xref  AS x ON f.id=x.fieldsid'
...
//                                <div>
//                                        <p>'.JText::_('COM_TZ_PINBOARD_FIELD_GROUP_DESC').'</p>
//                                </div>
//                            </div>
//                        </div>
//                    </div>';
...
//            $value   = implode(',',$value);
//
//            $query  = 'INSERT INTO #__tz_pinboard_tags(`name`,`published`)'
//                      .' VALUES '.$value;
//
//            $db     = &JFactory::getDbo();
//            $db -> setQuery($query);
...
//            }
//            $value  = implode(',',$value);
//            $query  = 'INSERT INTO #__tz_pinboard_tags_xref(`contentid`,`tagsid`)'
//                      .'VALUES '.$value;
//            $db -> setQuery($query);
//            if(!$db -> query()){
//                var_dump($db -> getErrorMsg());
...
//                $str    = $this -> imageUrl.'/cache/'.uniqid() .'tz_pinboard_'.time().'.'
...
//                    $str2       = null;
//                    if($task == 'save2copy'){
//                        $str2    = $this -> imageUrl.'/cache/'.uniqid() .'tz_pinboard_'.time().'.'
...
//        }
//
//        $checkfile_img = $data['tz_select_img'];
//
//
//        if($checkfile_img ==1){
...
//                require_once(JPATH_COMPONENT.DIRECTORY_SEPARATOR.'libraries'.DIRECTORY_SEPARATOR.'HTTPFetcher.php');
//                require_once(JPATH_COMPONENT.DIRECTORY_SEPARATOR.'libraries'.DIRECTORY_SEPARATOR.'readfile.php');
...
//
//                                $newImage -> toFile($destPath);
//                            }
//                           JFile::delete($desttamp);
//
//                $imageGallery->name = $this -> imageUrl.'/cache/'.$FileName_img;
//        }else{
//
//                            $curPath    = null;
//                            $str2       = null;
...
//
//                                $str2    = $this -> imageUrl.'/cache/'.uniqid() .'tz_pinboard_'.time().'.'
//                                  .JFile::getExt($edit_url_img);
//
//                                foreach($sizes as $key => $val){
//                                    $curPath    =  str_replace('.'.JFile::getExt($edit_url_img),'_'.$key.'.'.JFile::getExt($edit_url_img),$edit_url_img);
//                                    $curPath    = JPATH_SITE.DIRECTORY_SEPARATOR.str_replace('/',DIRECTORY_SEPARATOR,$curPath);
//
//                                    $destPath2  = JPATH_SITE.DIRECTORY_SEPARATOR.str_replace('/',DIRECTORY_SEPARATOR,$str2);
//                                    $destPath2  = str_replace('.'.JFile::getExt($destPath2),'_'.$key.'.'.JFile::getExt($destPath2),$destPath2);
...
//                            }
//                            else{
//                                $imageGallery -> name   = $edit_url_img;
//                            }
//
//            }
//        }
...
//                            $_type  = '.'.$_type;
//                            $str = $this -> imageUrl.'/cache/'.uniqid().'tz_pinboard_'.(time()).$_type;
//                            $arr2[$i]   = $str;
//
//                            // Check size
...
//                    }
//
//                    $str = $this -> imageUrl.'/cache/'.uniqid().'tz_pinboard_'.(time()+$i+1).'.'.JFile::getExt($item);
//                    $arr2[$i]   = $str;
//
//                    $curName    = null;
...
//                        if($task == 'save2copy'){
//
//                            $str2    = $this -> imageUrl.'/cache/thumbnail/'.uniqid() .'tz_pinboard_'.time().'.'
...
//                            }
//
//                            $fileName   = $this -> imageUrl.'/cache/thumbnail/'.uniqid().'tz_pinboard_'.(time()).'.'
...
//                        $fileName   = null;
//                        $_fileName   = uniqid().time().'tz_pinboard_'.$data['tz_media_code_youtube']
...
//                        $thumb      = $file -> get($thumbUrl);
//                        $fileName   = null;
//                        $_fileName  = uniqid().time().'tz_pinboard_'.$data['tz_media_code_vimeo'].'.'. JFile::getExt($thumbUrl);
//
                 ...
                 if(isset($link_url) && !empty($link_url)){
                       ...
                       $docUrl=new Services_Yadis_PlainHTTPFetcher();
                           if($contentURL=$docUrl->get($data)){
                              ...
                              if(preg_match_all('/<img.*?>/i', $contentURL->body, $match)){ // lay the img ve

                                  foreach($match[0] as $match_all){
                                        $match_all_img[] = $match_all; // gan gia tri tra ve vao bien $mall_all_img
                                  }
                                  $match_all_implode = implode('',$match_all_img); // chuyen ve chuoi
                                  if(preg_match_all('/src="http\:\/\/.*?"/i', $match_all_implode, $_mat2)){// lay ra src cua the img

                                                $mang_href = $_mat2[0];
                                  }
                                  if(count($mang_href) > 0){
                                                $chuyenchuoi_href = implode('',$mang_href);
                                                $chuyemang_href = explode('"src=',$chuyenchuoi_href);
                                                $chuyenchuoi_href_2 = implode("",$chuyemang_href);
                                                $chuyemang_href_2 = explode('"',$chuyenchuoi_href_2);

                                  } else if(preg_match_all('/\/\/www.*?"/i', $match_all_implode, $_mat2)){

                                                $chuyenchuoi_img = $_mat2[0];
                                      if(count($chuyenchuoi_img) >0){
                                                 $chuyenchuoi_href = implode('',$chuyenchuoi_img);
                                                 $chuyemang_href = explode('"src=',$chuyenchuoi_href);
                                                 $chuyenchuoi_href_2_1 = implode("",$chuyemang_href);
                                                 $chuyemang_href_2_0 = explode('"',$chuyenchuoi_href_2_1);
                                            for($i =0; $i < count($chuyemang_href_2_0); $i++){

                                                $chuyemang_href_2[] = 'http:'.$chuyemang_href_2_0[$i];

                                            }
                                            array_pop($chuyemang_href_2);
                                      }else if(preg_match_all('/\/images.*?"/i', $match_all_implode, $_mat2)){
                                                $chuyenchuoi_img = $_mat2[0];
                                                 $chuyenchuoi_href = implode('',$chuyenchuoi_img);
                                                 $chuyemang_href = explode('"src=',$chuyenchuoi_href);
                                                 $chuyenchuoi_href_2_1 = implode("",$chuyemang_href);
                                                 $chuyemang_href_2_0 = explode('"',$chuyenchuoi_href_2_1);
                                                for($i =0; $i < count($chuyemang_href_2_0); $i++){
                                                    $chuyemang_href_2[] = $link_url.$chuyemang_href_2_0[$i];
                                            ...
                                            $dem  = count($chuyemang_href_2);



                                             for($i=1; $i < $dem; $i++){

                                               if(isset($chuyemang_href_2[$i]) && !empty($chuyemang_href_2[$i])){

                                                  $src =  @getimagesize("$chuyemang_href_2[$i]"); // ham lay ve width va hiehg cua anh
                                                  if(is_array($src)){
...
//    function ajaxUrlImg(){
//
//                   require_once(JPATH_COMPONENT.DIRECTORY_SEPARATOR.'views'.DIRECTORY_SEPARATOR.'article'.DIRECTORY_SEPARATOR.'view.html.php'); // chen file view.html.php vao
//                   $view = new TZ_PinboardViewArticle();
//                    $view->assign('hienthi_img',$this->getWebsite());
//                    return  $view -> loadTemplate('edit_url');
//
//    }
